...
from common import log_on_error
...
class TenetBot:
    ...
    @classmethod
    async def create(cls, api_id: int, api_hash: str, token: str) -> "TenetBot":
        coroutine: Awaitable[TelegramClient] = (
            TelegramClient("Tenet Bot", api_id, api_hash).start(bot_token=token)
        )
        bot: TelegramClient = await coroutine
        self: TenetBot = cls(bot)
        return self
    ...
    async def process_video(
        ...
        width, height = map(int, subprocess.check_output(shlex.split(
            f"ffprobe -v error -select_streams v:0 -show_entries stream=width,height -of csv=s=x:p=0 {tempfile.name}"
        )).decode().split("x"))

        if max(width, height) > 1280:  # Downscale in case large resolution
        ...
        if actual_filter_mode == 0:
            command = (
                ...
                f"-i {tempfile.name} "
                '-filter_complex "'
                "[0:v]crop=in_w/2:in_h:in_w/2:0,reverse[r];"  # Reverse right part
                "[0:v][r]overlay=w:eof_action=pass"
                f'" {outfile.name}'
            )
            ...
            command = (
                ...
                "[0:v]crop=in_w/2:in_h:0:0,reverse[l];"     # Reverse left part
                "[0:v][l]overlay=w:eof_action=pass[wl];"    # Left part to right
                "[wl][r]overlay=eof_action=pass"            # Right part to left
                f'" {outfile.name}'
            )
        else:
...
async def main():
    ...
    bot = await TenetBot.create(api_id, api_hash, token)
    await bot.start_bot()
